...
async def to_code(config):
    ...
    if CONF_NONE in config:
        cg.add(ping.set_none(await binary_sensor.new_binary_sensor(config[CONF_NONE])))
    if CONF_SOME in config:
        cg.add(ping.set_some(await binary_sensor.new_binary_sensor(config[CONF_SOME])))
    if CONF_ALL in config:
        cg.add(ping.set_all(await binary_sensor.new_binary_sensor(config[CONF_ALL])))
    ...
    if CONF_SINCE in config:
        ...
        await since_.to_code(since_config)
    ...
    if CONF_TARGETS in config:
        for target_config in config[CONF_TARGETS]:
            ...
            if CONF_ABLE in target_config:
                cg.add(
                    target.set_able(
                        await binary_sensor.new_binary_sensor(target_config[CONF_ABLE])
                    )
                )
